...
from my_hello_agents.tools.my_calculator_tool import create_calculator_registry
...
def test_calculator_tool():
    ...
    # 创建包含计算器的注册表
    register = create_calculator_registry()
    ...
    for i, expression in enumerate(test_cases, 1):
        ...
        result = register.execute_tool("my_calculator", expression)
        print(f"结果: {result}\n")
...
def test_with_simple_agent():
    ...
    # think() 已在内部完成流式打印并返回完整字符串，直接使用即可
    print("\n")
